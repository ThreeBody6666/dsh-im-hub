...
export function createTelegramAdapter(options) {
		...
		try {
			...
			for (const update of updates ?? []) {
				...
				try {
					...
					logger?.warn?.(`dsh-im-gateway: telegram message handling failed: ${error instanceof Error ? error.message : String(error)}`);
				}
			}
		} catch (error) {
			logger?.warn?.(`dsh-im-gateway: telegram poll failed: ${error instanceof Error ? error.message : String(error)}`);
		} finally {
	...
	return {
		...
		async start() {
			...
			logger?.info?.(`dsh-im-gateway: telegram bot @${me.username ?? me.id} connected`);
			poll();
		},
		stop() {
			stopped = true;
