...
export function createWecomAdapter(options) {
	...
	async function handlePost(req, res, url) {
		...
		try {
			...
			logger?.warn?.(`dsh-im-gateway: wecom message handling failed: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
		...
		async start() {
			...
			server = createServer((req, res) => {
				...
				if (req.method === 'GET') handleGet(req, res, url);
					...
					logger?.warn?.(`dsh-im-gateway: wecom callback error: ${error instanceof Error ? error.message : String(error)}`);
					if (!res.headersSent) {
			...
			await new Promise((resolve, reject) => {
				...
				server.listen(port, host, () => {
					...
					logger?.info?.(`dsh-im-gateway: wecom callback server listening on http://${host}:${port}${path}`);
					resolve();
				});
			});
		},
