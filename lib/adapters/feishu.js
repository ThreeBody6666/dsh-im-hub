...
export function createFeishuAdapter(options) {
	...
	function dispatch(event) {
		...
		onMessage({
			...
			logger?.warn?.(`dsh-im-gateway: feishu message handling failed: ${error instanceof Error ? error.message : String(error)}`);
		});
	}
	...
	async function connectWs() {
		...
		try {
			...
			logger?.info?.(`dsh-im-gateway: feishu long connection obtained (service ${serviceId})`);

			// 2) Connect to the wss URL.
			ws = new WebSocket(url);
			ws.binaryType = 'arraybuffer';
			ws.onopen = () => {
				logger?.info?.('dsh-im-gateway: feishu websocket connected');
			...
			ws.onmessage = (ev) => {
				...
				try {
					...
					logger?.warn?.(`dsh-im-gateway: feishu frame decode failed: ${error instanceof Error ? error.message : String(error)}`);
					return;
				}
				if (frame.method === 1) {
			...
			ws.onclose = () => {
				clearInterval(pingTimer);
				if (!stopped) {
					logger?.warn?.('dsh-im-gateway: feishu websocket closed, reconnecting in 5s');
					wsRetryTimer = setTimeout(connectWs, 5000);
				}
			};
			ws.onerror = (ev) => {
				logger?.warn?.(`dsh-im-gateway: feishu websocket error: ${ev?.message ?? 'unknown'}`);
			};
		} catch (error) {
			logger?.warn?.(`dsh-im-gateway: feishu websocket connect failed: ${error instanceof Error ? error.message : String(error)}; retrying in 10s`);
	...
	async function startHttp() {
		...
		await new Promise((resolve, reject) => {
			...
			server.listen(port, host, () => {
				...
				logger?.info?.(`dsh-im-gateway: feishu webhook server listening on http://${host}:${port}${webhookPath}`);
				resolve();
			});
		});
	}
