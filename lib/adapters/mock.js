...
export function createMockAdapter(options) {
	...
	function handleText(text, chatId, userId) {
		...
		return onMessage({
			...
			logger?.warn?.(`dsh-im-gateway: mock message handling failed: ${error instanceof Error ? error.message : String(error)}`);
		});
	}
	...
	return {
		...
		async start() {
			...
			server.listen(port, '127.0.0.1', () => {
				...
				logger?.info?.(`dsh-im-gateway: mock adapter ready (stdin + http://127.0.0.1:${actual}/mock)`);
			});
		},
		stop() {
			stopped = true;
